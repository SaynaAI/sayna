...
Sayna supports Silero-VAD for audio-level voice activity detection to determine when a user has stopped speaking. This feature is gated behind the `stt-vad` Cargo feature, which includes both VAD and turn detection bundled together.

## Overview

The `stt-vad` feature provides an integrated VAD + turn detection system. VAD monitors audio for silence, and when continuous silence exceeds a configurable threshold, the ONNX-based turn detection model is triggered to confirm if the speaker's turn is complete.
...
## Enabling VAD + Turn Detection

### Build with Feature

```bash
# Build with VAD and turn detection support
...
**Note:** The `stt-vad` feature always includes both Silero-VAD for silence detection and the ONNX-based turn detection model. There is no separate `turn-detect` feature—VAD and turn detection are bundled together.

### Server Configuration (YAML)

When `stt-vad` is compiled, VAD is always active. Configure tuning parameters:
...
Configure VAD tuning parameters via the WebSocket config message:
...
|-------|------|---------|-------------|
| `enabled` | bool | — | **Ignored.** When `stt-vad` is compiled, VAD is always active. Kept for backward compatibility. |
...
## How VAD and Turn Detection Work Together

The `stt-vad` feature integrates VAD with turn detection in a single, cohesive system:
...
3. **Silence triggers evaluation**: When `TurnEnd` event fires, the turn detection model is invoked
4. **Model confirms turn**: The ONNX turn detection model analyzes the accumulated transcript
5. **speech_final emitted**: If the turn is complete, an artificial `speech_final` event is sent

### Why This Combined Approach?

- **VAD alone** can trigger on natural pauses mid-sentence
- **Turn detection alone** may wait too long if the STT provider doesn't send `speech_final`
- **The bundled approach** uses audio silence as a trigger, but text content as confirmation, providing the best of both worlds
...
- Confirm the server was built with `--features stt-vad`
- Verify the Silero-VAD ONNX model is available (check logs for download/load errors)
...
- [VAD + Turn Detection](../CLAUDE.md#voice-activity-detection-vad-with-turn-detection) - Full VAD and turn detection documentation
