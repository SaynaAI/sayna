...
impl LiveKitRoomHandler {
    ...
    /// Setup room recording with the configured storage backend.
    ///
    /// Recording uploads are wired through `RecordingConfig`, which produces
    /// the appropriate LiveKit `EncodedFileOutput` (S3 or GCS) so this method
    /// is backend-agnostic.
    ///
    /// # Arguments
    ...
    /// ```rust,no_run
    /// use sayna::livekit::room_handler::LiveKitRoomHandler;
    /// use sayna::config::{RecordingConfig, RecordingBackend, S3Backend};
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let handler = LiveKitRoomHandler::new(
    ///     "http://localhost:7880".to_string(),
    ...
    pub async fn setup_room_recording(
        ...
        let filepath = config.build_recording_filepath(stream_id);
        let file_output = config.build_egress_output(filepath);

        // Audio-only room composite — Sayna never publishes video tracks.
...
mod tests {
    ...
    fn test_recording_config_s3() -> RecordingConfig {
        use crate::config::{RecordingBackend, S3Backend};
        RecordingConfig {
    ...
    fn test_recording_configuration_validation() {
        ...
        let recording_config = test_recording_config_s3();
        ...
        let cfg = handler_with_recording
            .recording_config
            .as_ref()
            .expect("recording config attached");
    ...
    async fn test_setup_room_recording_with_config() {
        // Validates that setup_room_recording accepts a properly configured handler.
        // The actual API call fails because there is no real LiveKit server, but the
        // failure must not be a config-validation failure.
        let handler = LiveKitRoomHandler::new(
            "http://localhost:7880".to_string(),
            "test_key".to_string(),
            "test_secret".to_string(),
            Some(test_recording_config_s3()),
        )
        .unwrap();
