...
impl LiveKitWebSocketConfig {
    ...
    /// * `audio_enabled` - Whether websocket audio is enabled for this session
    /// * `tts_config` - Optional TTS configuration containing audio parameters
    ...
    pub fn to_livekit_config(
        ...
        let sample_rate = if audio_enabled {
            tts_config
        ...
        LiveKitConfig {
            ...
            // Use TTS config sample rate only when audio is enabled.
            sample_rate,
            // Assume mono audio for TTS (1 channel)
            channels: 1,
            // Noise filtering only matters when remote audio is processed.
            enable_noise_filter: audio_enabled && cfg!(feature = "noise-filter"),
