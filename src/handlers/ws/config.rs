...
impl LiveKitWebSocketConfig {
    ...
    ///
    /// Audio format parameters (sample_rate, channels) are derived from the TTS
    /// configuration since LiveKit publishes TTS output audio. This ensures the
    /// published audio track uses the correct format for TTS playback.
    ///
    /// If TTS sample_rate is not specified, defaults to 16000Hz (matching the
    /// common STT input rate) for consistency.
    ///
    /// Note: Input audio (STT path) uses a separate sample rate configured
    /// in the STT provider.
    ///
    /// # Arguments
    /// * `token` - JWT token for LiveKit room access (generated by LiveKitRoomHandler)
    /// * `tts_config` - TTS configuration containing audio parameters for output audio
    ...
    pub fn to_livekit_config(
        ...
        // Use TTS config for publishing audio format
        // Default to 16000Hz (STT rate) if TTS doesn't specify
        let sample_rate = tts_config.sample_rate.unwrap_or(16000);
        // Default to mono for TTS output
        let channels = 1u16;
