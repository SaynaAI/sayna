...
impl LiveKitWebSocketConfig {
    ...
    ///
    /// Audio format parameters (sample_rate, channels) are derived from the STT
    /// configuration since LiveKit receives input audio that feeds into the STT
    /// pipeline. This ensures the receive audio path uses a consistent format.
    ///
    /// Note: Noise filtering is handled centrally in `VoiceManager::receive_audio`,
    /// not in LiveKit config. LiveKit forwards raw audio to VoiceManager which
    /// applies noise filtering when enabled.
    ///
    /// # Arguments
    /// * `token` - JWT token for LiveKit room access (generated by LiveKitRoomHandler)
    /// * `stt_config` - STT configuration containing audio parameters for input audio
    ...
    pub fn to_livekit_config(
        ...
        LiveKitConfig {
            ...
            // Use STT config sample rate for input audio format
            sample_rate: stt_config.sample_rate,
            // Use STT config channels for input audio format
